...
export default function Dashboard() {
  ...
  const [isFetchingDataDeathsChart, setIsFetchingDataDeathsChart] = useState(true);
  const [chartDeathsData, setChartDeathsData] = useState({});

  const [chartInfectedByAge, setChartInfectedByAge] = useState({});
  const [isFetchingDataInfectedByAgeChart, setIsFetchingDataInfectedByAgeChart] = useState(true);

  const [chartInfectedByGenre, setChartInfectedByGenre] = useState({});
  const [isFetchingDataInfectedByGenreChart, setIsFetchingDataInfectedByGenreChart] = useState(true);
  ...
  useEffect(() => {
    service.getGraficoInfectadosPorIdade()
      .then(response => {
        let variavel = {
          labels: response.data.labels,
          datasets: [
  ...
  useEffect(() => {
    service.getGraficoInfectadosPorGenero()
      .then(response => {
        let variavel = {
          labels: response.data.labels,
          datasets: [
      ...
      <View style={styles.container}>
        ...
        {isFetchingDataDeathsChart ?
          ...
          <View style={[styles.chartContainer, { marginTop: 0 }]} >
            ...
            />
            <View style={styles.viewTitleChart}>
              <Text style={[{ fontWeight: 'bold' }, styles.txtChart]}>Índice de mortalidade:{" "}</Text>
              <Text style={styles.txtChart}>{formatValue(chartDeathsData.death_rate).replace(',', '.')}</Text>
            </View>
          </View>
        }
        ...
        <View style={{ flexDirection: 'column' }}>
          ...
          <Text style={{ color: '#1d253b', textAlign: 'justify', marginBottom: 20 }}> Os dados apresentados nos gráficos são referentes a cidade de Londrina, PR - Brasil.</Text>
          <Text style={{ color: '#1d253b', textAlign: 'justify', marginBottom: 20 }}>
            O número de casos confirmados e óbitos são oriundos dos boletins da{" "}
            <Text style={{ color: '#adb5bd', fontWeight: 'bold' }} onPress={() => { Linking.openURL('https://www.saude.pr.gov.br/Pagina/Coronavirus-COVID-19') }}>
          ...
          <Text style={{ color: '#1d253b', textAlign: 'justify', marginBottom: 20 }}>
            ...
            <Text style={{ color: '#adb5bd', fontWeight: 'bold' }} onPress={() => { Linking.openURL('https://opendatasus.saude.gov.br/dataset/casos-nacionais') }}>
              e-SUS NOTIFICA</Text>,{" "}
              que foi desenvolvido para registro de casos de Síndrome Gripal suspeitos de Covid-19,
                  e contém dados referentes ao local de residência do paciente,
                  independentemente de terem sido notificados em outro estado ou município,
                  além de demográficos e clínicos epidemiológicos dos casos.
          </Text>
          <Text style={{ color: '#525f7f', textAlign: 'justify', marginLeft: 15, fontSize: 12 }}>
            &#8226;{" "}Estes dados não são referentes ao número total de casos confirmados,
            são considerados apenas dados em que o resultado foi POSITIVO no teste para o Covid-19 dentro do dataset disponibilizado pelo e-SUS.
          </Text>
        </View>
      </View>
    </ScrollView >
